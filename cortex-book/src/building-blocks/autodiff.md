...
Cortex's tensor also supports auto-differentiation, which is an essential part of any deep learning
framework. We introduced the `Backend` trait in the [previous section](./backend.md), but Cortex also
...
With Cortex, you don't need to wrap the backend with the `Autodiff` for inference, and you
...
with optimizers from `cortex-core`. To work with the `Module` trait, a translation step is required to
