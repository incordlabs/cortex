...
Nearly everything in Cortex is based on the `Backend` trait, which enables you to run tensor
...
mode since cortex fully supports dynamic graphs. However, we may create another API to assist with
...
backend developers in mind rather than Cortex users. Therefore, most Cortex userland APIs are generic
