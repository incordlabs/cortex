...
Records are how states are saved with Cortex. Compared to most other frameworks, Cortex has its own
advanced saving mechanism that allows interoperability between backends with minimal possible
runtime errors. There are multiple reasons why Cortex decided to create its own saving formats.
...
validation code. Since modules in Cortex are created with configurations, they can't implement
...
only supports serializing and deserializing tensors. The record system in Cortex gives you the
