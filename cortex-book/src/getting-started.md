...
Cortex is a deep learning framework in the Rust programming language. Therefore, it goes without
...
thing to do to start using Cortex.

## Creating a Cortex application
...
```console
cargo new my_cortex_app
```

This will initialize the `my_cortex_app` project directory with a `Cargo.toml` file and a `src`
...
Then, add Cortex as a dependency:

```console
cargo add cortex --features wgpu
```
...
That's it, you're ready to start! You have a project configured with Cortex and the WGPU backend,
...
To bring any of the Cortex module or item into scope, a `use` declaration is added.
...
```rust, ignore
use cortex::{tensor::Tensor, backend::Wgpu};
```
...
Cortex comes with a variety of things in its core library. When creating a new model or using an
