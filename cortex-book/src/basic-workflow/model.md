...
In the `Cargo.toml` file, add the `cortex` dependency with `train`, `vision` and `wgpu` features.
...
cortex = { version = "~0.21", features = ["std", "tui", "train", "vision", "wgpu", "fusion"], default-features = false }
# cortex = { version = "~0.21", features = ["train", "vision", "wgpu"] }
```
...
2. Note that the struct is generic over the [`Backend`](../building-blocks/backend.md) trait. The
   ...
   In Cortex, the `Backend` trait enables you to run tensor operations using different implementations
