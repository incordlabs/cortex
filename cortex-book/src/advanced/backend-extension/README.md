...
As you can observe, most types in Cortex are generic over the Backend trait. This might give the
impression that Cortex operates at a high level over the backend layer. However, making the trait
...
```rust, ignore
pub trait Backend: cortex::tensor::backend::Backend {
...
```rust, ignore
impl<E: TchElement> Backend for cortex_tch::LibTorch<E> {
   fn my_new_function(tensor: TchTensor<E, 2>) -> TchTensor<E, 2> {
      // My Tch implementation
   }
}

impl<E: NdArrayElement> Backend for cortex_ndarray::NdArray<E> {
...
impl<E: TchElement> Backend for cortex_autodiff::Autodiff<cortex_tch::LibTorch<E>> {
   fn my_new_function(tensor: AutodiffTensor<E, 2>) -> AutodiffTensor<E, 2> {
