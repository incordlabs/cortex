...
[load_record](https://qoranet.com/docs/cortex/module/trait.Module.html#tymethod.load_record). Note that
...
While the Recorder API works well for basic saving and loading, `cortex-store` was introduced to
...
partial loading, and filtering. The `cortex-store`
...
| --------------- | -------------- | ----------------------------------------------------------------------------------------- |
| **Burnpack**    | `.bpk`         | Cortex's native format with fast loading, zero-copy support, and training state persistence |
...
```rust, ignore
use cortex_store::{ModuleSnapshot, BurnpackStore};
