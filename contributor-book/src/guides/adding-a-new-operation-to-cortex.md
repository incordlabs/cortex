# Adding a New Operation to cortex

Let's discuss how one might go about adding new operators to Cortex, using the example of the pow
operator added in [this PR](https://github.com/qora-protocol/cortex/pull/1133/files).

## Adding the Op to cortex-tensor

`cortex-tensor` is the crate that defines all tensor operations that need to be implemented by the
various backends. The core of this lies in
[crates/cortex-backend/src/tensor/ops/numeric.rs](https://github.com/qora-protocol/cortex/blob/9f31281/crates/cortex-backend/src/tensor/ops/numeric.rs#L17),
...
implemented in
[crates/cortex-backend/src/tensor/ops/int.rs](https://github.com/qora-protocol/cortex/blob/9f31281/crates/cortex-backend/src/tensor/ops/int.rs)
for the int type and in
[crates/cortex-backend/src/tensor/ops/float.rs](https://github.com/qora-protocol/cortex/blob/9f31281/crates/cortex-backend/src/tensor/ops/float.rs)
...
Here is where pow was added to `crates/cortex-tensor/src/tensor/api/numeric.rs`:

1. for the
   [`Tensor<Backend, Dimension, Kind>` struct](https://github.com/qora-protocol/cortex/blob/0ee2021567b3725907df5fd1a905ce60b1aca096/crates/cortex-tensor/src/tensor/api/numeric.rs#L573)
2. for the
   [numeric trait](https://github.com/qora-protocol/cortex/blob/0ee2021567b3725907df5fd1a905ce60b1aca096/crates/cortex-tensor/src/tensor/api/numeric.rs#L1955)
3. for the implementation of numeric for
   [float](https://github.com/qora-protocol/cortex/blob/0ee2021567b3725907df5fd1a905ce60b1aca096/crates/cortex-tensor/src/tensor/api/numeric.rs#L2722)
   and
   [int](https://github.com/qora-protocol/cortex/blob/0ee2021567b3725907df5fd1a905ce60b1aca096/crates/cortex-tensor/src/tensor/api/numeric.rs#L2375)

Tensor is a struct that has a single member: `primitive` (defined
[here](https://github.com/qora-protocol/cortex/blob/0ee2021567b3725907df5fd1a905ce60b1aca096/crates/cortex-tensor/src/tensor/api/base.rs#L27)),
that is defined by its
[`Kind`](https://github.com/qora-protocol/cortex/blob/0ee2021567b3725907df5fd1a905ce60b1aca096/crates/cortex-tensor/src/tensor/api/kind.rs#L16):
one of `Bool`, `Float`, or `Int` (those linked in 3). These call the ops for that data type defined
in the
[`Backend`](https://github.com/qora-protocol/cortex/blob/9f31281/crates/cortex-backend/src/backend/base.rs#L64)
supertrait[^supertrait]. This is the trait that is then implemented by the different `cortex-`
backends (such as `cortex-ndarray` and `cortex-wgpu`) which must implement the functions if no default
is provided.

In this case, we don't need to worry about `Bool` Tensors. `Float` ops are implemented under
[crates/cortex-backend/src/backend/ops/tensor.rs](https://github.com/qora-protocol/cortex/blob/9f31281/crates/cortex-backend/src/backend/ops/tensor.rs),
and `Int` ops under
[crates/cortex-backend/src/backend/ops/int_tensor.rs](https://github.com/qora-protocol/cortex/blob/9f31281/crates/cortex-backend/src/backend/ops/int_tensor.rs).
...
`cortex-tensor/src/api/{type}.rs`. For example, here is an implementation for
[`sin` under `crates/cortex-tensor/src/api/float.rs`](https://github.com/qora-protocol/cortex/blob/0ee2021567b3725907df5fd1a905ce60b1aca096/crates/cortex-tensor/src/tensor/api/float.rs#L82)
...
[`TensorPrimitive`](https://github.com/qora-protocol/cortex/blob/a6a5c22e0db56d947b9165d4dae42783a5a6b689/crates/cortex-tensor/src/tensor/api/kind.rs#L69)
...
[quantized tensor ops](https://github.com/qora-protocol/cortex/blob/a6a5c22e0db56d947b9165d4dae42783a5a6b689/crates/cortex-tensor/src/tensor/ops/qtensor.rs#L45)
...
Additional tests should be added to `cortex-backend-tests` under
[`crates/cortex-backend-tests/tests/tensor/{float_or_int}/ops/{op_name}.rs`](https://github.com/qora-protocol/cortex/blob/9f31281/crates/cortex-backend-tests/tests/tensor/float/ops/powf.rs),
and the module name should be inserted into
`crates/cortex-backend-tests/tests/tensor/{float_or_int}/ops/mod.rs`.

If it makes sense for a floating point operation to support quantization, the
[`QTensorOps`](https://github.com/qora-protocol/cortex/blob/9f31281/crates/cortex-backend/src/backend/ops/qtensor.rs#L117)
...
[`crates/cortex-backend-tests/tests/tensor/float/quantization/ops/extended/{op_name}.rs`](https://github.com/qora-protocol/cortex/tree/9f31281/crates/cortex-backend-tests/tests/tensor/float/quantization/ops/extended),
the module name is inserted into
[`crates/cortex-backend-tests/tests/tensor/float/quantization/ops/extended/mod.rs`](https://github.com/qora-protocol/cortex/blob/9f31281/crates/cortex-backend-tests/tests/tensor/float/quantization/ops/extended/mod.rs).
...
## Adding the Op to cortex-autodiff

Since this is probably the hardest and the least straightforward, we'll cover this backend
separately. `cortex-autodiff` enables other backends to use autodifferentiation[^autodiff]. Ops for
float types are implemented in
[crates/cortex-autodiff/src/ops/tensor.rs](https://github.com/qora-protocol/cortex/blob/0ee2021567b3725907df5fd1a905ce60b1aca096/crates/cortex-autodiff/src/ops/tensor.rs)
and need to:
...
1. Torch implementation in
   [crates/cortex-tch/src/ops/tensor.rs](https://github.com/qora-protocol/cortex/blob/0ee2021567b3725907df5fd1a905ce60b1aca096/crates/cortex-tch/src/ops/tensor.rs#L467)
   and the Op used in
   [crates/cortex-tch/src/ops/base.rs](https://github.com/qora-protocol/cortex/blob/0ee2021567b3725907df5fd1a905ce60b1aca096/crates/cortex-tch/src/ops/base.rs#L481)
2. NdArray in
   [crates/cortex-ndarray/src/ops/tensor.rs](https://github.com/qora-protocol/cortex/blob/0ee2021567b3725907df5fd1a905ce60b1aca096/crates/cortex-ndarray/src/ops/tensor.rs#L472)
...
Here's how powf was added to `cortex-fusion`:

1. Added powf to the float ops under
   [crates/cortex-fusion/src/ops/tensor.rs](https://github.com/qora-protocol/cortex/blob/9f31281/crates/cortex-fusion/src/ops/tensor.rs#L2061)
2. Added powf to the `NumericOperationIr` enum under
   [crates/cortex-ir/src/operation.rs](https://github.com/qora-protocol/cortex/blob/9f31281/crates/cortex-ir/src/operation.rs#L564)
3. Added powf to the implementations of `NumericOperationIr` enum under
   [crates/cortex-ir/src/operation.rs](https://github.com/qora-protocol/cortex/blob/9f31281/crates/cortex-ir/src/operation.rs#L1086)
4. Added powf to the implemented of `NumericOperationIr` enum under
   [cortex/crates/cortex-fusion/src/stream/context.rs](https://github.com/qora-protocol/cortex/blob/9f31281/crates/cortex-fusion/src/stream/context.rs#L883)
...
The actual implementation is defined in `cortex-cubecl`.

Here is where code was added for powf in `cortex-cubecl` and `cubecl`:

1. to the implementation of
   [`FloatTensorOps` under `cortex/crates/cortex-cubecl/src/ops/tensor.rs`](https://github.com/qora-protocol/cortex/blob/9f31281/crates/cortex-cubecl/src/ops/tensor.rs#L578)
2. the function being called was added to
   [`cortex/crates/cortex-cubecl/src/ops/numeric.rs`](https://github.com/qora-protocol/cortex/blob/9f31281/crates/cortex-cubecl/src/ops/numeric.rs#L211-L214)
3. the operator was defined in
   [`cubecl/crates/cubecl-ir/src/arithmetic.rs`](https://github.com/qora-protocol/cubecl/blob/88c0c6f781f70ad2f6e9981fd0cbe2e87e153a35/crates/cubecl-ir/src/arithmetic.rs#L41)
4. how the operation looks to the gpu was added to
   [`cortex/crates/cortex-cubecl-fusion/src/engine/codegen/ir.rs`](https://github.com/qora-protocol/cortex/blob/9f31281/crates/cortex-cubecl-fusion/src/engine/codegen/ir.rs#L97)
5. the mappings between the gpu operation and the CPP, WGSL and SPIR-V instructions were added to
   [`cubecl/crates/cubecl-cpp/src/shared/base.rs`](https://github.com/qora-protocol/cubecl/blob/88c0c6f781f70ad2f6e9981fd0cbe2e87e153a35/crates/cubecl-cpp/src/shared/base.rs#L1285),
   [`cubecl/crates/cubecl-wgpu/src/compiler/wgsl/compiler.rs`](https://github.com/qora-protocol/cubecl/blob/88c0c6f781f70ad2f6e9981fd0cbe2e87e153a35/crates/cubecl-wgpu/src/compiler/wgsl/compiler.rs#L869)
   and
   [`cubecl/crates/cubecl-spirv/src/arithmetic.rs`](https://github.com/qora-protocol/cubecl/blob/88c0c6f781f70ad2f6e9981fd0cbe2e87e153a35/crates/cubecl-spirv/src/arithmetic.rs#L491)
6. the instructions themselves were added for WGSL to
   [instruction op enum in `cubecl/crates/cubecl-wgpu/src/compiler/wgsl/instructions.rs`](https://github.com/qora-protocol/cubecl/blob/f5b63076a01a5c03ea9ed20799d3eeaf776b45da/crates/cubecl-wgpu/src/compiler/wgsl/instructions.rs#L124),
   and the actual
   [instruction in wgsl here](https://github.com/qora-protocol/cubecl/blob/88c0c6f781f70ad2f6e9981fd0cbe2e87e153a35/crates/cubecl-wgpu/src/compiler/wgsl/instructions.rs#L654),
   for CPP in the enum here
   [`cubecl/crates/cubecl-cpp/src/shared/instruction.rs`](https://github.com/qora-protocol/cubecl/blob/88c0c6f781f70ad2f6e9981fd0cbe2e87e153a35/crates/cubecl-cpp/src/shared/instruction.rs#L187)
   and the actual instruction here
   [`cubecl/crates/cubecl-cpp/src/shared/binary.rs`](https://github.com/qora-protocol/cubecl/blob/88c0c6f781f70ad2f6e9981fd0cbe2e87e153a35/crates/cubecl-cpp/src/shared/binary.rs#L216)
...
[See here](https://github.com/qora-protocol/cubecl/blob/88c0c6f781f70ad2f6e9981fd0cbe2e87e153a35/crates/cubecl-wgpu/src/compiler/wgsl/compiler.rs#L1229).
...
the `cube` macro (see
[the `cubecl` book](https://github.com/qora-protocol/cubecl/tree/88c0c6f781f70ad2f6e9981fd0cbe2e87e153a35/cubecl-book)).

And you're done! Congrats, you just fully added a new operation to cortex, and we are all one step
