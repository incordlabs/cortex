...
use cortex::Tensor;
use cortex::collective::{AllReduceStrategy, CollectiveConfig, ReduceOperation};
use cortex::prelude::{Backend, DeviceOps};
use cortex::tensor::Shape;
use cortex::tensor::backend::DeviceId;
...
fn run(args: &Args) -> Result<(), Box<dyn Error + Send + Sync + 'static>> {
    run_backend::<cortex::backend::Cuda>(args)
}

#[cfg(feature = "metal")]
fn run(args: &Args) -> Result<(), Box<dyn Error + Send + Sync + 'static>> {
    run_backend::<cortex::backend::Metal>(args)
}

#[cfg(feature = "wgpu")]
fn run(args: &Args) -> Result<(), Box<dyn Error + Send + Sync + 'static>> {
    run_backend::<cortex::backend::Wgpu>(args)
}

#[cfg(feature = "ndarray")]
fn run(args: &Args) -> Result<(), Box<dyn Error + Send + Sync + 'static>> {
    run_backend::<cortex::backend::ndarray>(args)
}
