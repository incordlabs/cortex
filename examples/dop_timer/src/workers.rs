...
use cortex::Tensor;
use cortex::collective::{CollectiveConfig, PeerId, ReduceOperation, all_reduce};
use cortex::prelude::Backend;
use cortex::tensor::TensorPrimitive;
...
impl<B: Backend> Worker<B> {
    ...
    pub fn run(&mut self, rx: Receiver<WorkRequest<B>>) {
        ...
        while let Ok(command) = rx.recv() {
            use cortex::collective::register;
            match command {
                RegisterRequest { tx } => {
