...
Converts between different weight formats (PyTorch or Safetensors) to Cortex's native Burnpack format.
...
Demonstrates loading and using a model from Cortex's native Burnpack format.
...
Example:
```bash
cargo run --bin burnpack -- 35 /tmp/cortex-convert/mnist
...
2. Use the `convert` example to convert to Cortex's native Burnpack format
