...
use cortex_backend::{DType, Shape, TensorData, TensorMetadata, backend::ExecutionError};
use cortex_ir::{TensorId, TensorIr, TensorStatus};
    ...
    fn drop(&mut self) {
        ...
        match self.status(count) {
            TensorStatus::ReadWrite => {
                ...
                self.client.register_op(cortex_ir::OperationIr::Drop(ir));
            }
            TensorStatus::ReadOnly => {}
