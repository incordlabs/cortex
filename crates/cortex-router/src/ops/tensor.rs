...
use cortex_backend::backend::{Backend, ExecutionError};

use crate::{BackendRouter, RunnerChannel, RunnerClient, get_client};
use cortex_backend::tensor::{
...
impl<R: RunnerChannel> FloatTensorOps<Self> for BackendRouter<R> {
    ...
    fn float_cast(tensor: FloatTensor<Self>, dtype: cortex_backend::FloatDType) -> FloatTensor<Self> {
        let client = tensor.client.clone();
