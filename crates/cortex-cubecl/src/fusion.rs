...
use cortex_cubecl_fusion::optim::reduce::ReduceSettings;
use cortex_cubecl_fusion::optim::reduce_broadcasted::ReduceBroadcastedFuser;
use cortex_cubecl_fusion::{
...
use cortex_ir::{BackendIr, TensorHandle};
use cortex_std::Metadata;
use core::marker::PhantomData;
use std::sync::Arc;

impl<R, BT> cortex_fusion::Optimization<FusionCubeRuntime<R, BT>> for CubeOptimization<R>
where
    R: CubeRuntime,
    BT: BoolElement,
{
    fn execute(
        &mut self,
        context: &mut cortex_fusion::stream::Context<
            '_,
...
{
    fn run(&self, context: &mut cortex_fusion::stream::Context<'_, CubeFusionHandle<R>>) {
...
impl<R: CubeRuntime, BT: BoolElement> FusionRuntime for FusionCubeRuntime<R, BT> {
    ...
    fn fusers(device: R::Device) -> Vec<Box<dyn cortex_fusion::OperationFuser<Self::Optimization>>> {
        vec![
