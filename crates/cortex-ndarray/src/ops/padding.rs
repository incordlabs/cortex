...
pub(crate) fn apply_padding_4d<E: NdArrayElement>(
    ...
    x_new = NdArrayOps::slice_assign(
        x_new,
        &[
            cortex_backend::Slice::from(0..batch_size),
            cortex_backend::Slice::from(0..input_channels),
            cortex_backend::Slice::from(padding_height..height + padding_height),
            cortex_backend::Slice::from(padding_width..width + padding_width),
        ],
        x,
    );
...
pub(crate) fn apply_padding_5d<E: NdArrayElement>(
    ...
    x_new = NdArrayOps::slice_assign(
        ...
        &[
            cortex_backend::Slice::from(0..batch_size),
            cortex_backend::Slice::from(0..input_channels),
            cortex_backend::Slice::from(padding_depth..depth + padding_depth),
            cortex_backend::Slice::from(padding_height..height + padding_height),
            cortex_backend::Slice::from(padding_width..width + padding_width),
        ],
        x,
