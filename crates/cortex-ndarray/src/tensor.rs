...
macro_rules! execute_with_dtype {
    (($lhs:expr, $rhs:expr),$element:ident,  $op:expr, [$($dtype: ident => $ty: ty),*]) => {{
        let lhs_dtype = cortex_backend::TensorMetadata::dtype(&$lhs);
        let rhs_dtype = cortex_backend::TensorMetadata::dtype(&$rhs);
        match ($lhs, $rhs) {
            $(
...
mod utils {
    use cortex_std::tensor::is_contiguous;
...
impl QTensorPrimitive for NdArrayQTensor {
    ...
    fn default_scheme() -> QuantScheme {
        QuantScheme::default().with_store(cortex_backend::quantization::QuantStore::Native)
    }
}
...
mod tests {
    ...
    use cortex_std::rand::get_seeded_rng;
