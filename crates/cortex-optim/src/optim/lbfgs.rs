...
use cortex::module::{AutodiffModule, Module, ModuleMapper, ModuleVisitor, Param};
use cortex::prelude::ToElement;
use cortex::record::Record;
...
mod tests {
    ...
    use cortex::tensor::{Tensor, TensorData};
    ...
    fn test_lbfgs_strong_wolfe_comparison() {
        ...
        let mut closure = |mod_in: Linear<TestAutodiffBackend>| {
            ...
            let loss = cortex_nn::loss::MseLoss::new().forward(
                output,
                y_true.clone(),
                cortex_nn::loss::Reduction::Sum,
            );
