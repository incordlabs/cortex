...
use cortex_backend::backend::{Backend, DeviceId, DeviceOps, ExecutionError};
use cortex_backend::ops::IntTensorOps;
...
impl cortex_backend::Device for LibTorchDevice {
...
/// [cortex-tch](https://github.com/qora-protocol/cortex/tree/main/crates/cortex-tch).
///
...
impl<E: TchElement> Backend for LibTorch<E> {
    ...
    fn sync(device: &Self::Device) -> Result<(), ExecutionError> {
        match device {
            ...
            _ => {
                ...
                cortex_backend::read_sync(Self::int_into_data(Self::int_zeros(
                    [1].into(),
                    device,
                    E::dtype().into(),
                )))
    ...
    fn dtype_usage(
        _device: &Self::Device,
        dtype: cortex_backend::DType,
    ) -> cortex_backend::DTypeUsageSet {
        if dtype.try_into_kind().is_ok() {
            cortex_backend::DTypeUsage::general()
        } else {
            cortex_backend::DTypeUsageSet::empty()
        }
    }
}
