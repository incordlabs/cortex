use cortex_backend::{Shape, TensorMetadata};
use tch::Scalar;
...
impl TchOps {
    ...
    pub fn slice_with_steps(tensor: TchTensor, slices: &[cortex_backend::Slice]) -> TchTensor {
    ...
    fn generate_slice_indices(slice: &cortex_backend::Slice, dim_size: usize) -> Vec<i64> {
        let step = slice.step;
        let range = slice.to_range(dim_size);
