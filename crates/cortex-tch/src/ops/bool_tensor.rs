...
use cortex_backend::tensor::BoolTensor;
use cortex_backend::tensor::IntTensor;
use cortex_backend::{Shape, TensorData, TensorMetadata, ops::BoolTensorOps};

impl<E: TchElement> BoolTensorOps<Self> for LibTorch<E> {
    fn bool_from_data(data: TensorData, device: &LibTorchDevice) -> TchTensor {
        match data.dtype {
            cortex_backend::DType::Bool => TchTensor::from_data::<bool>(data, (*device).into()),
    ...
    fn bool_slice(tensor: TchTensor, slices: &[cortex_backend::Slice]) -> TchTensor {
