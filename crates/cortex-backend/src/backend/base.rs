use cortex_std::DType;
pub use cortex_std::backtrace::BackTrace;
...
/// This trait defines all types and functions needed for a backend to be used with cortex.
///
/// ## Design
///
...
/// ### Eager Mode
///
/// Because cortex supports dynamic graphs, the backend trait is designed around kernel
...
/// with cortex. This pattern could also be used to create an operation fusion trait, which
...
/// struct in the `cortex-tensor` crate.
/// For modules, public functions are often created, which can be used by `cortex-core` modules.
pub trait Backend:
    FloatTensorOps<Self>
    + BoolTensorOps<Self>
    + IntTensorOps<Self>
...
pub enum ExecutionError {
    ...
    /// A generic error happened during execution thrown in the Cortex project.
    ///
