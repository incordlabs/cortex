...
fn test_mean_dim_fused_on_read_on_write() {
    // https://github.com/qora-protocol/cortex/issues/3987
    let device = Default::default();
    let x = TestTensor::ones([128, 32, 1], &device);

    let weight = TestTensor::ones([1, 32, 1], &device);
    let options = cortex_tensor::ops::ConvOptions::new([1], [0], [1], 1);
    let x = cortex_tensor::module::conv1d(x, weight, None, options);
...
fn test_multiple_reduce_dims_permuted() {
    // Regression test for https://github.com/qora-protocol/cortex/issues/4461
