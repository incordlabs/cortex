...
use cortex_backend::Backend;
use cortex_backend::ExecutionError;
use cortex_std::DType;

#[cfg(feature = "autodiff")]
use cortex_autodiff::grads::Gradients;
#[cfg(feature = "autodiff")]
use cortex_backend::AutodiffBackend;

use crate::backends::*;
use crate::{DispatchDevice, DispatchTensor};

/// The main execution backend in Cortex.
///
...
/// in a backend-agnostic way. It allows Cortex to provide a unified, global backend
...
impl Backend for Dispatch {
    ...
    fn dtype_usage(device: &Self::Device, dtype: DType) -> cortex_backend::DTypeUsageSet {
