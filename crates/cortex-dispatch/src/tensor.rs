use cortex_backend::{Backend, QTensorPrimitive, TensorMetadata, tensor::FloatTensor};
...
impl<B: Backend> TensorMetadata for BackendTensor<B> {
    fn dtype(&self) -> cortex_std::DType {
        match self {
    ...
    fn shape(&self) -> cortex_std::Shape {
        match self {
...
impl<B: Backend> QTensorPrimitive for BackendTensor<B> {
    fn scheme(&self) -> &cortex_std::QuantScheme {
        match self {
