...
// https://github.com/qora-protocol/cortex/issues/3642

impl FloatTensorOps<Self> for Dispatch {
    fn float_from_data(
        data: cortex_backend::TensorData,
    ...
    fn float_to_device(tensor: FloatTensor<Self>, device: &DispatchDevice) -> FloatTensor<Self> {
        float_to_device!(
            ...
            |inner, device| {
                let data =
                    cortex_backend::read_sync(B1::float_into_data(inner)).expect("Should read data");
    ...
    fn float_grid_sample_2d(
        ...
        options: cortex_backend::ops::GridSampleOptions,
    ) -> FloatTensor<Self> {
