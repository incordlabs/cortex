...
use cortex_backend::{Scalar, ops::unfold::calculate_unfold_windows};
use cortex_std::{FloatDType, Shape, Slice};
...
impl<B: Backend, C: CheckpointStrategy> FloatTensorOps<Self> for Autodiff<B, C> {
    ...
    fn float_random(
        shape: Shape,
        distribution: cortex_backend::Distribution,
        device: &Device<Self>,
    ) -> FloatTensor<Self> {
        ...
        impl<B: Backend> Backward<B, 1> for CumProd {
            ...
            fn backward(
                ...
                unary::<B, _>(ops.parents, ops.node, grads, |grad| {
                    ...
                    // See: https://github.com/qora-protocol/cortex/issues/3864
    ...
    fn float_cast(tensor: FloatTensor<Self>, dtype: cortex_std::FloatDType) -> FloatTensor<Self> {
        #[derive(Debug)]
        struct Cast;
    ...
    // https://github.com/qora-protocol/cortex/issues/1458
