...
use cortex_communication::Address;
...
use cortex::{backend::NdArray, prelude::Backend, tensor::Tensor};
use cortex_collective::{AllReduceStrategy, ReduceOperation};
use cortex_collective_multinode_tests::shared::{NodeTest, NodeTestResult, TENSOR_RANK};
use cortex_std::rand::{SeedableRng, StdRng};
...
fn generate_random_input(
    ...
    let input: Vec<TensorData> = (0..input_count)
        .map(|_| {
            TensorData::random::<f32, _, _>(shape, cortex::tensor::Distribution::Default, &mut rng)
        })
        .collect();
