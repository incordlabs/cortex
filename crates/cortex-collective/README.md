...
This name is confusing in the context of tensors, so in cortex-collective the participating units
are called "peers".
...
Start one easily with `cortex_collective::start_global_orchestrator()`.

On the global level, nodes use the `cortex_communication::data_service::TensorDataService` to
