...
mod tests {
    ...
    use cortex_ndarray::NdArrayDevice;
    ...
    impl<B: Backend> Linear<B> {
        pub fn new(in_features: usize, out_features: usize, device: &B::Device) -> Self {
            let weight = Tensor::random(
                ...
                cortex_tensor::Distribution::Default,
                device,
            );
            let bias = Tensor::random([out_features], cortex_tensor::Distribution::Default, device);
    ...
    fn test_tensor_serde() {
        let tensor: cortex_tensor::Tensor<TestBackend, 1> =
            cortex_tensor::Tensor::ones([1], &NdArrayDevice::default());
        let encoded = serde_json::to_string(&tensor).unwrap();
        let decoded: cortex_tensor::Tensor<TestBackend, 1> = serde_json::from_str(&encoded).unwrap();
