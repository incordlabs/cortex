...
impl<T: Parameter> Param<T> {
    ...
    /// Gets a reference to the parameter value without cloning.
    ///
    /// Like `val()`, initializes lazily if needed. Unlike `val()`, returns `&T`
    /// instead of a cloned `T`, avoiding potentially expensive deep copies.
    ...
    /// by `cortex` and should handle those details.
