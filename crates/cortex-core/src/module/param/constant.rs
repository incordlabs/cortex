...
macro_rules! constant {
    (module) => {
        type Record = cortex::module::ConstantRecord;

        fn visit<V: cortex::module::ModuleVisitor<B>>(&self, _visitor: &mut V) {
            // Nothing to do
        }

        fn map<M: cortex::module::ModuleMapper<B>>(self, _mapper: &mut M) -> Self {
            self
        }
        ...
        fn into_record(self) -> Self::Record {
            cortex::module::ConstantRecord::new()
        }
        ...
        fn collect_devices(&self, devices: cortex::module::Devices<B>) -> cortex::module::Devices<B> {
            devices
        }
    };
    ...
    ($type:ty) => {
        impl<B: cortex::tensor::backend::Backend> cortex::module::Module<B> for $type {
            constant!(module);
        }

        impl<B: cortex::tensor::backend::AutodiffBackend> cortex::module::AutodiffModule<B> for $type {
            constant!(ad_module, $type);
        }

        impl cortex::module::ModuleDisplayDefault for $type {
            fn content(&self, content: cortex::module::Content) -> Option<cortex::module::Content> {
                let string = format!("{}", self);
                content.add_formatted(&string).optional()
            }
        }

        impl cortex::module::ModuleDisplay for $type {}
    };
}
...
impl cortex::module::ModuleDisplay for str {}
impl cortex::module::ModuleDisplayDefault for str {
    fn content(&self, content: cortex::module::Content) -> Option<cortex::module::Content> {
...
mod tests {
    ...
    use cortex_tensor::{Device, Tensor};
