...
mod tests {
    ...
    fn test_dataloader_slice_multi_device() {
        ...
        let (device1, device2) = (
            cortex_ndarray::NdArrayDevice::Cpu,
            cortex_ndarray::NdArrayDevice::Cpu,
        );

        #[cfg(all(test, feature = "test-tch"))]
        let (device1, device2) = (
            cortex_tch::LibTorchDevice::Cuda(0),
            cortex_tch::LibTorchDevice::Cuda(1),
        );

        #[cfg(all(test, feature = "test-wgpu"))]
        let (device1, device2) = (
            cortex_wgpu::WgpuDevice::DiscreteGpu(0),
            cortex_wgpu::WgpuDevice::DiscreteGpu(1),
        );

        #[cfg(all(test, feature = "test-cuda"))]
        let (device1, device2) = (cortex_cuda::CudaDevice::new(0), cortex_cuda::CudaDevice::new(1));
