    ...
    Send + Sync + core::default::Default + core::fmt::Debug + Clone
{
        ...
        R: Record<B>,
    {
        ...
        let item = CortexRecord::new::<Self>(item);
    ...
    where
        R: Record<B>,
    {
        let item: CortexRecord<R::Item<Self::Settings>, B> =
            self.load_item(&mut args).map_err(|err| {
                if let Ok(record) = self.load_item::<CortexRecordNoItem>(&mut args) {
                    ...
                    if metadata.version != record.metadata.version {
                        message += format!(
                            "\nMetadata has a different Cortex version: Actual {:?}, Expected {:?}",
...
fn recorder_metadata<R, B>() -> CortexMetadata
where
    R: Recorder<B>,
    B: Backend,
{
...
pub struct CortexMetadata {
    ...
    /// Cortex record version used to record the item.
    pub version: String,
...
pub struct CortexRecord<I, B: Backend> {
...
impl<I, B: Backend> CortexRecord<I, B> {
...
mod tests {
    static FILE_PATH: &str = "/tmp/cortex_test_record";

    use crate::TestBackend;

    use super::*;
    use cortex_tensor::{Device, ElementConversion};
