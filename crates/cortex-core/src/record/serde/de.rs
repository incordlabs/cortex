...
use super::{adapter::CortexModuleAdapter, error::Error};
...
pub struct Deserializer<A: CortexModuleAdapter> {
...
impl<A: CortexModuleAdapter> Deserializer<A> {
...
impl<'de, A: CortexModuleAdapter> serde::Deserializer<'de> for Deserializer<A> {
    type Error = Error;
...
struct VecSeqAccess<A: CortexModuleAdapter, I> {
...
impl<A: CortexModuleAdapter> VecSeqAccess<A, NestedValue> {
...
impl<A: CortexModuleAdapter> VecSeqAccess<A, u8> {
    fn new(vec: NestedValue, default_for_missing_fields: bool) -> Self {
        match vec {
...
impl<A: CortexModuleAdapter> VecSeqAccess<A, u16> {
    fn new(vec: NestedValue, default_for_missing_fields: bool) -> Self {
        match vec {
...
impl<A: CortexModuleAdapter> VecSeqAccess<A, f32> {
    fn new(vec: NestedValue, default_for_missing_fields: bool) -> Self {
        match vec {
...
struct HashMapAccess<A: CortexModuleAdapter> {
...
impl<A: CortexModuleAdapter> HashMapAccess<A> {
...
struct ProbeEnumAccess<A: CortexModuleAdapter> {
    value: NestedValue,
...
impl<A: CortexModuleAdapter> ProbeEnumAccess<A> {
    ...
    A: CortexModuleAdapter,
{
    ...
    fn unit_variant(self) -> Result<(), Self::Error> {
        ...
        match self.value {
            ...
            _ => unimplemented!(
                "unit variant is not implemented because it is not used in the cortex module"
            ),
        }
    }
    ...
    where
        V: Visitor<'de>,
    {
        unimplemented!("tuple variant is not implemented because it is not used in the cortex module")
    }
    ...
    {
        unimplemented!(
            "struct variant is not implemented because it is not used in the cortex module"
        )
    }
}

/// A wrapper for the nested value data structure with a cortex module adapter.
struct NestedValueWrapper<A: CortexModuleAdapter> {
    value: NestedValue,
    default_for_missing_fields: bool,
    phantom: std::marker::PhantomData<A>,
}

impl<A: CortexModuleAdapter> NestedValueWrapper<A> {
...
impl<A: CortexModuleAdapter> IntoDeserializer<'_, Error> for NestedValueWrapper<A> {
