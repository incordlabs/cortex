    ...
    fn gen_item_type(
        ...
        for variant in self.variants.iter() {
            ...
            variants.extend(quote! {
                ...
                #name(<#ty as cortex::record::Record<B>>::Item<S>),
            });

            // Item types must implement serialization/deserialization
            serde_bounds.extend(quote! {
                <#ty as cortex::record::Record<B>>::Item<S>: cortex::serde::Serialize + cortex::serde::de::DeserializeOwned,
            });
            clone_bounds.push(parse_quote! {
                <#ty as cortex::record::Record<B>>::Item<S>: Clone
            });
        ...
        if !has_backend {
            let param: syn::TypeParam = parse_quote! { B: cortex::tensor::backend::Backend };
        ...
        quote! {
            ...
            #[derive(cortex::serde::Serialize, cortex::serde::Deserialize)]
            #[serde(crate = "cortex::serde")]
    ...
    fn gen_into_item(&self, _item_name: &Ident) -> TokenStream {
        ...
        for variant in self.variants.iter() {
            ...
            into_item_match_arms.extend(quote! {
                Self::#name(record) => Self::Item::#name(cortex::record::Record::<B>::into_item::<S>(record)),
            });
        }

        quote! {
            fn into_item<S: cortex::record::PrecisionSettings>(self) -> Self::Item<S> {
                match self {
    ...
    fn gen_from_item(&self) -> TokenStream {
        ...
        for variant in self.variants.iter() {
            ...
            from_item_match_arms.extend(quote! {
                Self::Item::#name(item) => Self::#name(cortex::record::Record::<B>::from_item::<S>(item, device)),
            });
        }

        quote! {
            fn from_item<S: cortex::record::PrecisionSettings>(item: Self::Item<S>, device: &B::Device) -> Self {
                match item {
