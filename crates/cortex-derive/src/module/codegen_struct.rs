...
impl ModuleCodegen for StructModuleCodegen {
    ...
    fn gen_num_params(&self) -> TokenStream {
        let body = self.gen_fields_fn(|name| {
            quote! {
                num_params += cortex::module::Module::<B>::num_params(&self.#name);
            }
        });
    ...
    fn gen_visit(&self) -> TokenStream {
        ...
        let body = self.gen_fields_fn(|name| {
            ...
            quote! {
                ...
                cortex::module::Module::visit(&self.#name, visitor);
                visitor.exit_module(#name_str, #container_type);
            }
        });

        quote! {
            fn visit<Visitor: cortex::module::ModuleVisitor<B>>(&self, visitor: &mut Visitor) {
                #body
            }
        }
    }
    ...
    fn gen_collect_devices(&self) -> TokenStream {
        let body = self.gen_fields_fn(|name| {
            quote! {
                let devices = cortex::module::Module::<B>::collect_devices(&self.#name, devices);
            }
        });
        ...
        quote! {
            fn collect_devices(
                &self,
                devices: cortex::module::Devices<B>
            ) -> cortex::module::Devices<B> {
                #body
    ...
    fn gen_to_device(&self) -> TokenStream {
        let (names, body) = self.gen_fields_fn_names(|name| {
            quote! {
                let #name = cortex::module::Module::<B>::to_device(self.#name, device);
            }
        });
    ...
    fn gen_fork(&self) -> TokenStream {
        let (names, body) = self.gen_fields_fn_names(|name| {
            quote! {
                let #name = cortex::module::Module::<B>::fork(self.#name, device);
            }
        });
    ...
    fn gen_map(&self) -> TokenStream {
        ...
        let (names, body) = self.gen_fields_fn_names(|name| {
            ...
            quote! {
                ...
                let #name = cortex::module::Module::<B>::map(self.#name, mapper);
                mapper.exit_module(#name_str, #container_type);
            }
        });

        quote! {
            fn map<Mapper: cortex::module::ModuleMapper<B>>(self, mapper: &mut Mapper) -> Self {
                #body
    ...
    fn gen_valid(&self) -> TokenStream {
        let (names, body) = self.gen_fields_fn_names(|name| {
            quote! {
                let #name = cortex::module::AutodiffModule::<B>::valid(&self.#name);
            }
        });
    ...
    fn gen_from_inner(&self) -> TokenStream {
        let (names, body) = self.gen_fields_fn_names(|name| {
            quote! {
                let #name = cortex::module::AutodiffModule::<B>::from_inner(#name);
            }
        });
    ...
    fn gen_into_record(&self) -> TokenStream {
        let body = self.gen_fields_fn(|name| {
            quote! {
                #name: cortex::module::Module::<B>::into_record(self.#name),
            }
        });
    ...
    fn gen_load_record(&self) -> TokenStream {
        let body = self.gen_fields_fn(|name| {
            quote! {
                #name: cortex::module::Module::<B>::load_record(self.#name, record.#name),
            }
        });
