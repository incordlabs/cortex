...
pub(crate) fn generate_module_standard<Codegen: ModuleCodegen>(
    ...
    let mut codegen = quote! {
        impl #generics_module cortex::module::Module<B> for #name #generics_ty_module #generics_where_module {
        ...
        impl #generics_module_autodiff cortex::module::AutodiffModule<B> for #name #generics_ty_module_autodiff #generics_where_module_autodiff
        {
        ...
        impl #generics_module_has_autodiff cortex::module::HasAutodiffModule<B> for #name<B::InnerBackend, #generics_ty_train_module> #generics_where_module_has_autodiff
        {
        ...
        impl #generics_module cortex::module::ModuleDisplayDefault for #name #generics_ty_module #generics_where_module {
            #attributes_fn

            fn num_params(&self) -> usize {
                cortex::module::Module::num_params(self)
            }
        }
    ...
    if !has_custom_display(&ast.attrs) {
        codegen.extend(quote! {
            impl #generics_module cortex::module::ModuleDisplay for #name #generics_ty_module #generics_where_module {
...
pub(crate) fn generate_module_const(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let backend: syn::Generics = parse_quote! { <B: cortex::tensor::backend::Backend >};
    let backend_ad: syn::Generics = parse_quote! { <B: cortex::tensor::backend::AutodiffBackend >};
    ...
    let mut codegen = quote! {
        impl #generics_module cortex::module::Module<B> for #name #generics_ty #generics_where {
            cortex::constant!(module);
        }

        impl #generics_module_ad cortex::module::AutodiffModule<B>
            for #name #generics_ty #generics_where {
            cortex::constant!(ad_module, #name #generics_ty);
        }
        ...
        impl #generics cortex::module::ModuleDisplayDefault for #name #generics_ty #generics_where {
            #attributes_fn
        }
    ...
    if !has_custom_display(&ast.attrs) {
        codegen.extend(quote! {
            impl  #generics cortex::module::ModuleDisplay for #name #generics_ty #generics_where {
...
impl GenericsParser {
    fn from_ast(generics: &Generics) -> Self {
        ...
        module_autodiff.add_predicate(parse_quote! {
                B: cortex::tensor::backend::AutodiffBackend
        });

        module_autodiff.add_predicate(parse_quote! {
                <B as cortex::tensor::backend::AutodiffBackend>::InnerBackend: #backend_trait
        });
        ...
        .for_each(|ident| {
            module.add_predicate(
                parse_quote! {
                    #ident: cortex::module::Module<B>
                }
            );

            module.add_predicate(
                parse_quote! {
                    #ident: cortex::module::ModuleDisplay
                }
            );

            module_autodiff.add_predicate(
                parse_quote! {
                    #ident: cortex::module::AutodiffModule<B>
                }
            );

            module_autodiff.add_predicate(
                parse_quote! {
                    <#ident as cortex::module::AutodiffModule<B>>::InnerModule: cortex::module::Module<B::InnerBackend>
                }
            );

            module_autodiff.add_predicate(
                parse_quote! {
                    <#ident as cortex::module::AutodiffModule<B>>::InnerModule: cortex::module::ModuleDisplay
                }
            );

            generics_names_except_backend.extend(quote! { <#ident as cortex::module::AutodiffModule<B>>::InnerModule, });
            ...
            module_has_autodiff.add_predicate(
                parse_quote! {
                    #ident: cortex::module::Module<B::InnerBackend>
                }
            );
            ...
            module_has_autodiff.add_predicate(
                parse_quote! {
                    #ident: cortex::module::HasAutodiffModule<B>
                }
            );

            module_has_autodiff.add_predicate(
                parse_quote! {
                    #ident::TrainModule: cortex::module::ModuleDisplay
                }
            );
