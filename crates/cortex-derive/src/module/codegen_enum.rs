...
impl ModuleCodegen for EnumModuleCodegen {
    ...
    fn gen_num_params(&self) -> TokenStream {
        let match_body = self.gen_variants_match_fn(|_| {
            quote! {
                cortex::module::Module::<B>::num_params(module)
            }
        });
            ...
            quote! {
                {
                    ...
                    cortex::module::Module::visit(module, visitor);
    ...
    fn gen_collect_devices(&self) -> TokenStream {
        let match_body = self.gen_variants_match_fn(|_| {
            quote! {
                cortex::module::Module::<B>::collect_devices(module, devices)
            }
        });
    ...
    fn gen_to_device(&self) -> TokenStream {
        let match_body = self.gen_variants_match_fn(|variant| {
            quote! {
                Self::#variant(cortex::module::Module::<B>::to_device(module, device))
            }
        });
    ...
    fn gen_fork(&self) -> TokenStream {
        let match_body = self.gen_variants_match_fn(|variant| {
            quote! {
                Self::#variant(cortex::module::Module::<B>::fork(module, device))
            }
        });
            ...
            quote! {
                {
                    ...
                    let result = cortex::module::Module::<B>::map(module, mapper);
    ...
    fn gen_valid(&self) -> TokenStream {
        let match_body = self.gen_variants_match_fn(|variant| {
            quote! {
                Self::InnerModule::#variant(cortex::module::AutodiffModule::<B>::valid(module))
            }
        });
    ...
    fn gen_from_inner(&self) -> TokenStream {
        let match_body =
            self.gen_variants_match_fn_param("module", "Self::InnerModule::", |variant| {
                quote! {
                    Self::#variant(cortex::module::AutodiffModule::<B>::from_inner(module))
                }
            });
    ...
    fn gen_into_record(&self) -> TokenStream {
        let match_body = self.gen_variants_match_fn(|variant| {
            quote! {
                Self::Record::#variant(cortex::module::Module::<B>::into_record(module))
            }
        });
            ...
            quote! {
                {
                    ...
                    Self::#variant(cortex::module::Module::<B>::load_record(module, r))
                }
            }
        });
