...
use cortex_core::module::Module;
use cortex_core::prelude::*;
use cortex_core::record::{FullPrecisionSettings, NamedMpkFileRecorder, Recorder};
// use cortex_import::pytorch::{LoadArgs, PyTorchFileRecorder};
// use cortex_import::safetensors::SafetensorsFileRecorder;
use cortex_nn as nn;
use cortex_store::{
    BurnpackStore, ModuleSnapshot, PyTorchToCortexAdapter, PytorchStore, SafetensorsStore,
};
...
type NdArrayBackend = cortex_ndarray::NdArray<f32>;

#[cfg(feature = "wgpu")]
type WgpuBackend = cortex_wgpu::Wgpu;

#[cfg(feature = "cuda")]
type CudaBackend = cortex_cuda::Cuda<f32, i32>;

#[cfg(feature = "tch")]
type TchBackend = cortex_tch::LibTorch<f32>;

#[cfg(feature = "metal")]
type MetalBackend = cortex_wgpu::Metal;
...
fn generate_cortex_formats(st_path: &Path, bp_path: &Path, mpk_path: &Path) {
    type TestBackend = NdArrayBackend;
    let device = Default::default();

    // Load the model from SafeTensors
    let mut model = LargeModel::<TestBackend>::new(&device);
    let mut store = SafetensorsStore::from_file(st_path).with_from_adapter(PyTorchToCortexAdapter);
    model
        .load_from(&mut store)
    ...
    match check_model_files() {
        Ok(()) => {
            ...
            if !bp_path.exists() || !mpk_path.exists() {
                ...
                generate_cortex_formats(&st_path, &bp_path, &mpk_path);
            }
