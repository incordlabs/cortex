...
fn resnet18_path() -> PathBuf {
    ...
    let config_file = temp_dir.join("cortex_resnet18_benchmark").join("path.txt");
    ...
    temp_dir
        .join("cortex_resnet18_benchmark")
        .join("resnet18.pth")
}
    ...
    bencher.bench_local(|| {
        ...
        for key in &keys {
            ...
            let sum = match data.dtype {
                cortex_tensor::DType::F32 => data
                    .as_slice::<f32>()
                    .map(|s| s.iter().sum::<f32>())
                    .unwrap_or(0.0) as f64,
                cortex_tensor::DType::F64 => data
                    .as_slice::<f64>()
