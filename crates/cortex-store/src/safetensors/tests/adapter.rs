...
use crate::{CortexToPyTorchAdapter, ModuleSnapshot, PyTorchToCortexAdapter, SafetensorsStore};
use cortex_core::module::{Module, Param};
use cortex_nn::{Linear, LinearConfig};
use cortex_tensor::Tensor;
use cortex_tensor::backend::Backend;

type TestBackend = cortex_ndarray::NdArray;
...
#[test]
fn pytorch_to_cortex_adapter_linear_transpose() {
    let device = Default::default();
    let model = TestModel::<TestBackend>::new(&device);

    // Save with CortexToPyTorch adapter (will transpose linear weights)
    let mut save_store = SafetensorsStore::from_bytes(None).with_to_adapter(CortexToPyTorchAdapter);
    model.save_into(&mut save_store).unwrap();

    // Load with PyTorchToCortex adapter (will transpose back)
    let mut load_store = SafetensorsStore::from_bytes(None).with_from_adapter(PyTorchToCortexAdapter);
...
#[test]
fn pytorch_to_cortex_adapter_norm_rename() {
    let device = Default::default();
    ...
    // Save with CortexToPyTorch adapter (will rename gamma->weight, beta->bias)
    let mut save_store = SafetensorsStore::from_bytes(None).with_to_adapter(CortexToPyTorchAdapter);
    model.save_into(&mut save_store).unwrap();
    ...
    // Load with PyTorchToCortex adapter (will rename weight->gamma, bias->beta)
    let mut load_store = SafetensorsStore::from_bytes(None).with_from_adapter(PyTorchToCortexAdapter);
    if let SafetensorsStore::Memory(ref mut p) = load_store
        && let SafetensorsStore::Memory(ref p_save) = save_store
    {
...
fn adapter_with_pytorch_import() {
    use crate::PyTorchToCortexAdapter;

    let device = Default::default();

    // Reference the safetensors file from cortex-store
    ...
    // Load with PyTorchToCortex adapter
