...
fn multi_layer_model_import() {
    ...
    // Reference the safetensors file from cortex-import
    ...
    // Note: PyTorch and Cortex have different conventions for linear layer weights
    // PyTorch stores as [out_features, in_features], Cortex as [in_features, out_features]
    ...
    let mut store = SafetensorsStore::from_file(safetensors_path)
        .with_from_adapter(crate::PyTorchToCortexAdapter) // Use adapter to handle PyTorch format
    ...
    // Note: Due to shape mismatches (PyTorch vs Cortex conventions for linear layers),
