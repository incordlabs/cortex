...
use cortex_core::module::{Module, Param, ParamId};
use cortex_nn as nn;
use cortex_tensor::{Bool, Int, Tensor, backend::Backend};
...
mod tests {
    ...
    fn test_mixed_dtypes_round_trip() {
        type TestBackend = cortex_ndarray::NdArray<f32>;
        ...
        for snapshot in snapshots {
            ...
            if path.contains("float_tensor") || path.contains("linear") {
                ...
                assert!(
                    matches!(
                        dtype,
                        cortex::tensor::DType::I64
                            | cortex::tensor::DType::I32
                            | cortex::tensor::DType::I16
                            | cortex::tensor::DType::I8
                    ),
    ...
    fn test_mixed_precision_floats() {
        ...
        // (F16, BF16, F32, F64, etc.) in the same file, Cortex's backend architecture currently
        ...
        {
            type TestBackend = cortex_ndarray::NdArray<f64>;
            let device = Default::default();
    ...
    fn test_mixed_precision_integers() {
        ...
        struct MultiIntModel<B: Backend> {
            // Note: Cortex's Tensor<B, D, Int> uses the backend's default int type
    ...
    fn test_comprehensive_mixed_types() {
        ...
        let mut loaded_model = ComprehensiveModel::<TestBackend> {
            ...
            boolean_mask: Param::initialized(
                ParamId::new(),
                Tensor::from_bool(
                    cortex::tensor::TensorData::new(vec![false; 8], [2, 4]),
                    &device,
                ),
            ),
        };
