//! SafeTensors format support for Cortex deep learning framework.
//!
...
//!
//! ```rust,ignore
//! use cortex_store::{SafetensorsStore, ModuleSnapshot};
//!
...
//!
//! ```rust,no_run
//! # use cortex_store::{SafetensorsStore, KeyRemapper};
...
//!     .add_pattern(r"^pytorch\.(.*)", "cortex.$1").expect("valid regex")           // pytorch.layer -> cortex.layer
...
//!
//! ```rust,ignore
//! use cortex_store::{SafetensorsStore, ModuleSnapshot, PyTorchToCortexAdapter, CortexToPyTorchAdapter};
//!
//! // Loading PyTorch model into Cortex
//! let mut store = SafetensorsStore::from_file("pytorch_model.safetensors")
//!     .with_from_adapter(PyTorchToCortexAdapter)  // Transposes linear weights, renames norm params
//!     .allow_partial(true);                     // PyTorch models may have extra tensors
//!
//! let mut cortex_model = Model::new(&device);
//! cortex_model.load_from(&mut store)?;
//!
//! // Saving Cortex model for PyTorch
//! let mut store = SafetensorsStore::from_file("for_pytorch.safetensors")
//!     .with_to_adapter(CortexToPyTorchAdapter);   // Transposes weights back, renames for PyTorch
//!
//! cortex_model.save_into(&mut store)?;
//! ```
//!
...
//! ## Adapter Methods
//!
//! - **`with_from_adapter(adapter)`** - Set adapter for loading (e.g., PyTorchToCortexAdapter)
//! - **`with_to_adapter(adapter)`** - Set adapter for saving (e.g., CortexToPyTorchAdapter)
//!
...
//!
//! ```rust,no_run
//! use cortex_store::{SafetensorsStore, PyTorchToCortexAdapter};
//!
...
//!
//! Migrating a PyTorch model to Cortex with filtering, remapping, and adapters:
//!
//! ```rust,ignore
//! use cortex_store::{SafetensorsStore, ModuleSnapshot, PyTorchToCortexAdapter};
//!
...
//!     .metadata("converted_by", "cortex-store");
//!
