...
//!
//! This module provides adapters that handle differences between PyTorch and Cortex:
...
mod module_names {
    ...
    use cortex_nn::{BatchNorm, GroupNorm, LayerNorm, Linear};
...
pub trait ModuleAdapter: Send + Sync {
    ...
    /// with different naming conventions (e.g., PyTorch's "weight" vs Cortex's "gamma").
    ///
    /// # Arguments
    ...
    /// (e.g. PyTorch -> Cortex layout conversion, then dtype casting, then custom remapping).
    ///
...
/// Adapter for converting from PyTorch format to Cortex format
///
/// Handles:
/// - Linear layer weight transposition (PyTorch: [out, in] → Cortex: [in, out])
/// - Normalization parameter renaming (weight → gamma, bias → beta)
#[derive(Debug, Clone, Default)]
pub struct PyTorchToCortexAdapter;

impl ModuleAdapter for PyTorchToCortexAdapter {
    fn adapt(&self, snapshot: &TensorSnapshot) -> TensorSnapshot {
        adapt_pytorch_tensor(snapshot, PyTorchConversionDirection::PyTorchToCortex)
    }

    fn get_alternative_param_name(&self, param_name: &str, container_type: &str) -> Option<String> {
        // For PyTorch->Cortex: When looking for Cortex names (gamma/beta), try PyTorch names (weight/bias)
        if is_normalization_layer(container_type) {
            cortex_norm_param_to_pytorch(param_name).map(|s| s.to_string())
        } else {
            None
        }
    }
...
/// Adapter for converting from Cortex format to PyTorch format
///
/// Handles:
/// - Linear layer weight transposition (Cortex: [in, out] → PyTorch: [out, in])
/// - Normalization parameter renaming (gamma → weight, beta → bias)
#[derive(Debug, Clone, Default)]
pub struct CortexToPyTorchAdapter;

impl ModuleAdapter for CortexToPyTorchAdapter {
    fn adapt(&self, snapshot: &TensorSnapshot) -> TensorSnapshot {
        adapt_pytorch_tensor(snapshot, PyTorchConversionDirection::CortexToPyTorch)
    }

    fn get_alternative_param_name(&self, param_name: &str, container_type: &str) -> Option<String> {
        // For Cortex->PyTorch: When looking for PyTorch names (weight/bias), try Cortex names (gamma/beta)
        if is_normalization_layer(container_type) {
...
/// Map PyTorch normalization parameter name to Cortex
...
/// Map Cortex normalization parameter name to PyTorch
fn cortex_norm_param_to_pytorch(param_name: &str) -> Option<&'static str> {
    match param_name {
        "gamma" => Some("weight"),
        "beta" => Some("bias"),
        _ => None,
...
fn adapt_pytorch_tensor(
    ...
    if is_normalization_layer(&module_type) {
        let new_name = match direction {
            PyTorchConversionDirection::PyTorchToCortex => pytorch_norm_param_to_burn(param_name),
            PyTorchConversionDirection::CortexToPyTorch => cortex_norm_param_to_pytorch(param_name),
        };
...
mod tests {
    ...
    fn create_test_snapshot(path: &str, shape: Vec<usize>, container_type: &str) -> TensorSnapshot {
        ...
        TensorSnapshot::from_closure(
            ...
            cortex_core::module::ParamId::new(),
        )
    }

    #[test]
    fn test_pytorch_to_cortex_linear_weight() {
        let adapter = PyTorchToCortexAdapter;
    ...
    #[test]
    fn test_pytorch_to_cortex_norm_params() {
        let adapter = PyTorchToCortexAdapter;
    ...
    #[test]
    fn test_cortex_to_pytorch_linear_weight() {
        let adapter = CortexToPyTorchAdapter;
    ...
    #[test]
    fn test_cortex_to_pytorch_norm_params() {
        let adapter = CortexToPyTorchAdapter;
