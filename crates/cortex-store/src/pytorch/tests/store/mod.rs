...
/// Path to cortex-store test data files
...
mod adapter_tests {
    ...
    fn test_pytorch_adapter_always_applied() {
        // Test that PyTorchToCortexAdapter is always applied internally
        let path = pytorch_test_path("linear", "linear.pt");
        ...
        // PyTorchToCortexAdapter is always applied internally
        assert!(
            result.is_ok(),
            "Failed to load with internal PyTorchToCortexAdapter: {:?}",
            result.err()
        );
    ...
    fn test_pytorch_adapter_with_filtering() {
        // Test that PyTorchToCortexAdapter works with filtering
        let path = pytorch_test_path("linear", "linear.pt");
...
mod enum_variant_tests {
    ...
    /// Model with enum field that will have variant names in Cortex paths
    #[derive(Module, Debug)]
    ...
    fn test_enum_variant_path_mismatch() {
        ...
        // Cortex paths:    "feature.BaseConv.weight", "feature.BaseConv.bias", "classifier.weight", "classifier.bias"
        //                         ^^^^^^^^ enum variant name is included in Cortex but not PyTorch
...
mod map_indices_contiguous_tests {
    ...
    fn test_load_non_contiguous_indexes_with_mapping() {
        ...
        // The Cortex model expects fc.0, fc.1, fc.2, ... (contiguous)
