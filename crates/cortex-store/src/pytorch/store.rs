...
use crate::{
    ApplyResult, KeyRemapper, ModuleSnapshot, ModuleStore, PathFilter, PyTorchToCortexAdapter,
...
/// with automatic weight transformation using `PyTorchToCortexAdapter`.
...
impl PytorchStore {
    ...
    /// For example, PyTorch path "feature.weight" can match Cortex path "feature.BaseConv.weight".
    ///
...
impl ModuleStore for PytorchStore {
    ...
    fn apply_to<B: Backend, M: ModuleSnapshot<B>>(
        ...
        // Apply to module with PyTorchToCortexAdapter (always used for PyTorch files)
        // This adapter handles:
        // - Transposing linear weights from PyTorch format to Cortex format
        ...
        let result = module.apply(
            snapshots,
            filter_opt,
            Some(Box::new(PyTorchToCortexAdapter)),
