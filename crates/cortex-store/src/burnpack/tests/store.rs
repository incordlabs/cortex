...
use cortex_tensor::{Tensor, backend::Backend};
...
struct ForwardTestModel<B: Backend> {
    linear1: cortex_nn::Linear<B>,
    linear2: cortex_nn::Linear<B>,
}
...
impl<B: Backend> ForwardTestModel<B> {
    ...
    fn forward(&self, input: Tensor<B, 2>) -> Tensor<B, 2> {
        ...
        let x = cortex::tensor::activation::gelu(x);
        self.linear2.forward(x)
    }
}

#[derive(cortex::config::Config, Debug)]
...
impl ForwardTestModelConfig {
    fn init<B: Backend>(&self, device: &B::Device) -> ForwardTestModel<B> {
        ForwardTestModel {
            linear1: cortex_nn::LinearConfig::new(self.input_size, self.hidden_size)
                .with_bias(true)
                .init(device),
            linear2: cortex_nn::LinearConfig::new(self.hidden_size, self.output_size)
                .with_bias(true)
                .init(device),
        }
    }
...
fn test_forward_pass_preservation_after_save_load() {
    ...
    let input = Tensor::<TestBackend, 2>::random(
        [1, 4],
        cortex_tensor::Distribution::Uniform(-1.0, 1.0),
        &device,
    );
...
/// Regression test for https://github.com/qora-protocol/cortex/issues/4179
#[test]
fn test_store_quantized_module_round_trip() {
    use cortex_core::module::Quantizer;
    use cortex_nn::LinearConfig;
    use cortex_tensor::quantization::{
    ...
    let scheme = <<TestBackend as cortex_tensor::backend::Backend>::QuantizedTensorPrimitive as QTensorPrimitive>::default_scheme()
