...
use cortex_tensor::{Bytes, TensorData};
...
impl BurnpackReader {
    ...
    pub(crate) fn from_file_mmap<P: AsRef<Path>>(path: P) -> Result<Self, BurnpackError> {
        ...
        let bytes = Bytes::from_shared(shared_bytes, cortex_tensor::AllocationProperty::File);
            ...
            let data_fn: Rc<dyn Fn() -> Result<TensorData, crate::TensorSnapshotError>> =
                if zero_copy {
                    ...
                    Rc::new(move || {
                        ...
                        // See https://github.com/qora-protocol/cortex/pull/3792#discussion_r2416812091
