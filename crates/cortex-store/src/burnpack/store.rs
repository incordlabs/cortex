...
use alloc::vec::Vec;
use cortex_core::prelude::Backend;
use cortex_tensor::Bytes;
...
/// BurnpackStore - A Cortex-specific file format store using CBOR for metadata
...
impl BurnpackStore {
    ...
    pub fn default_metadata() -> BTreeMap<String, String> {
        ...
        metadata.insert("producer".into(), "cortex".into());
    ...
    ///
    /// ```no_run
    /// # use cortex_store::BurnpackStore;
    ...
    pub fn from_static(data: &'static [u8]) -> Self {
        use cortex_tensor::AllocationProperty;
...
impl ModuleStore for BurnpackStore {
    ...
    fn apply_to<B: Backend, M: ModuleSnapshot<B>>(
        ...
        // Burnpack is Cortex's native format, so no enum variant skipping needed
