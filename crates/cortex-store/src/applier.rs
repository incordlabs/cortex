...
use cortex_core::module::{ModuleMapper, Param};
use cortex_tensor::{Bool, Int, Shape, Tensor, backend::Backend};
...
impl<B: Backend> Applier<B> {
    ...
    where
        K: cortex_tensor::TensorKind<B>,
        K: cortex_tensor::BasicOps<B>,
    {
            ...
            && let Some(module_type) = self.current_module_type()
        {
            ...
            if let Some(alt_name) = adapter.get_alternative_param_name(param_name, module_type) {
                ...
                *alt_path_stack.last_mut().unwrap() = alt_name.clone();
                ...
                // Don't mark the alternative path as visited - only the original Cortex path
...
mod tests {
    use super::*;
    use cortex_core::module::{ModuleMapper, Param, ParamId};
    use cortex_tensor::{DType, Tensor, TensorData};
    ...
    fn dtype_preservation_f64() {
        ...
        type TestBackendF64 = cortex_ndarray::NdArray<f64>;
        let device = Default::default();
