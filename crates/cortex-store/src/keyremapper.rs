...
mod tests {
    ...
    fn create_test_tensor_snapshot(name: &str) -> TensorSnapshot {
        let data = TensorData {
            bytes: cortex_tensor::Bytes::from_bytes_vec(vec![1, 2, 3, 4]),
            shape: vec![2, 2],
            dtype: cortex_tensor::DType::F32,
        };
    ...
    fn test_key_remapper_from_patterns() {
        let patterns = vec![(r"^pytorch\.", "cortex."), (r"\.bias$", ".bias_param")];
        ...
        assert!(
            remapped
                .iter()
                .any(|v| v.full_path() == "cortex.linear.bias_param")
        );
    }
