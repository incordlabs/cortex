...
use cortex_backend::Scalar;
pub use cortex_backend::tensor::BasicOps;
...
use cortex_std::{SliceOps, stub::RwLock};
    ...
    K::Elem: Element,
{
    ...
    /// Returns a reference to the underlying primitive tensor.
    pub fn primitive_ref(&self) -> &K::Primitive {
        &self.primitive
    }
    ...
    /// ```rust
    /// use cortex_tensor::backend::Backend;
    /// use cortex_tensor::{Tensor, Shape};
    ///
    /// fn example<B: Backend>() {
    ...
    /// ```rust
    /// use cortex_tensor::backend::Backend;
    /// use cortex_tensor::{Tensor, Shape, s};
    ///
    /// fn example<B: Backend>() {
    ///     let device = B::Device::default();
    ///
    ///     // Single dimension slicing - no brackets needed!
    ///     let tensor = Tensor::<B, 1, cortex_tensor::Int>::arange(0..10, &device);
    ...
    /// ```rust
    /// use cortex_tensor::backend::Backend;
    /// use cortex_tensor::{Tensor, Shape, Bool};
    ///
    /// fn example<B: Backend>() {
    ///   let device = B::Device::default();
    ///   let tensor = Tensor::<B, 2>::from_data([[1.0, -2.0, 3.0], [5.0, 9.0, 6.0]], &device);
    ...
    <K as BasicOps<B>>::Elem: Debug,
{
    ...
    fn fmt_inner_tensor(
        ...
        for i in start..end {
            ...
            let data = cortex_std::reader::try_read_sync(self.clone().slice(range).into_data_async());
