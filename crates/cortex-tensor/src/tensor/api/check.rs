...
use alloc::vec;
use alloc::vec::Vec;
use cortex_backend::tensor::Ordered;
...
pub(crate) fn unwrap_shape_reshape(result: Result<Shape, cortex_std::MetadataError>) -> Shape {
    match result {
        Ok(shape) => shape,
        // `shape.reshape(new_shape)` should only return `MetadataError::Invalid`.
        Err(cortex_std::MetadataError::Invalid { reason }) => {
            macros::check!({
