...
use cortex_backend::{DeviceId, DeviceOps, ExecutionError, Shape, TensorData};
use cortex_communication::{Address, ProtocolClient, data_service::TensorTransferId};
use cortex_ir::TensorIr;
use cortex_router::{MultiBackendBridge, RouterTensor, RunnerClient, get_client};
use cortex_std::{backtrace::BackTrace, future::DynFut};
...
impl RunnerClient for RemoteClient {
    ...
    fn register_op(&self, op: cortex_ir::OperationIr) {
        self.sender
    ...
    fn create_empty_handle(&self) -> cortex_ir::TensorId {
        self.sender.new_tensor_id()
    }

    fn dtype_usage(&self, dtype: cortex_std::DType) -> cortex_backend::DTypeUsageSet {
...
impl cortex_std::device::Device for RemoteDevice {
